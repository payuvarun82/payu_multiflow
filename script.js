         ...
         * =======================
         * Enables/disables action buttons based on required field completion
         */
        ...
        function checkRequiredFields(flow) {
            ...
            if (flow === 'crossborder') {
                ...
                const subSection = document.getElementById('cb-subscription-section');
                if (subSection && subSection.classList.contains('active')) {
            ...
            if (useCustomKeys && useCustomKeys.checked) {
                ...
                if (!customKey || !customKey.value.trim() || 
                    !customSalt || !customSalt.value.trim() ||
                    !txnidDisplay || !txnidDisplay.value.trim()) {
                    return false;
                }
            }
        ...
        function updateButtonStates(flow) {
            const prefix = getFlowPrefix(flow);
            const isValid = checkRequiredFields(flow);
            
            // Get all buttons for this flow
            const debugBtn = document.getElementById(prefix + '-debug-btn');
            const curlBtn = document.getElementById(prefix + '-curl-btn');
            const codeBtn = document.getElementById(prefix + '-code-btn');
            const payBtn = document.getElementById(prefix + '-pay-btn');
            
            // Enable or disable buttons based on validation
            if (debugBtn) debugBtn.disabled = !isValid;
            if (curlBtn) curlBtn.disabled = !isValid;
            if (codeBtn) codeBtn.disabled = !isValid;
            if (payBtn) payBtn.disabled = !isValid;
        }
        
        function initializeButtonStateListeners(flow) {
            const prefix = getFlowPrefix(flow);
            
            // Common fields to monitor
            const fieldsToMonitor = [
                'amount', 'productinfo', 'firstname', 'email', 'phone',
                'address1', 'city', 'state', 'country', 'zipcode', 'lastname',
                'udf5_input', 'billing_amount', 'payment_start_date', 'payment_end_date',
                'billing_cycle', 'billing_interval', 'beneficiary_account', 'ifsc_code',
                'custom_key', 'custom_salt', 'txnid_display'
            ];
            ...
            // Also monitor custom keys checkbox
            const useCustomKeys = document.getElementById(prefix + '_use_custom_keys');
            if (useCustomKeys) {
                useCustomKeys.addEventListener('change', function() {
                    updateButtonStates(flow);
                });
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize button state listeners for all flows
            const allFlows = ['crossborder', 'nonseamless', 'subscription', 'tpv', 'upiotm', 'preauth', 'checkoutplus', 'split', 'bankoffer'];
            allFlows.forEach(function(flow) {
                initializeButtonStateListeners(flow);
            });
        ...
        function generateTemplate(flow) {
            ...
            // Update button states after filling sample data
            updateButtonStates(flow);
        }
